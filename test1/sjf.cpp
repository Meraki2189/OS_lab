...
int sjf(char *ch)
{
    ...
    int ready1[10]; //按照服务时间排序的进程在pcb1的位置
    int order1[10]; //记录服务时间的排序
    for(int i=0;i<k;i++){
        order1[i]=pcb1[ready[i]].ser_time;
        ready1[i]=ready[i];
    }
    int time = order[0]; //第一个进程进入时间
    int temp_p; //存储到达的进程数
    for(int l=0;l<k;l++){
        // 判断time已经到达的进程数
        for(int i=0;i<k&&pcb1[ready[i]].arr_time<=time;i++)
            temp_p = i+1;
        // 将到达的进程按照服务时间排序，以及把执行结束的进程放到后面
        for(int i=0;i<temp_p;i++)
            for(int j=i+1;j<temp_p;j++){
                if(order1[i]>order1[j]&&order1[j]!=0||order1[i]==0)
                {
                    int temp = order1[i];
                    order1[i] = order1[j];
                    order1[j] = temp;
